...
# Entity-matching embedding model — switched from Ollama's nomic-embed-text to a
# local SapBERT model (cambridgeltl/SapBERT-from-PubMedBERT-fulltext), which is
# purpose-built for medical/pharmacogenomic synonym alignment (see the MindMap
# fuzzy-alignment benchmark, nmmmm.py). Both keyword and KG-entity-name embeddings
# are computed live, in-process, with this same model — no HTTP embedding server
# required.
EMBEDDING_MODEL_NAME = os.environ.get(
    "EMBEDDING_MODEL_NAME", "cambridgeltl/SapBERT-from-PubMedBERT-fulltext"
)

# Robust device assignment for the local embedding model (same logic as nmmmm.py).
if torch.cuda.is_available():
    try:
        _test_tensor = torch.zeros(1).cuda()
        del _test_tensor
        torch.cuda.empty_cache()
        DEVICE = torch.device("cuda")
    except Exception as e:
        print(f"[Warning] CUDA detected but memory allocation failed: {e}. Falling back to CPU.", flush=True)
        DEVICE = torch.device("cpu")
else:
    DEVICE = torch.device("cpu")

# Minimum cosine similarity to accept a match at all, rather than forcing a
# best-available-but-wrong pick. NOTE: this value (0.50) is the SapBERT-calibrated
# default from the MindMap fuzzy-alignment benchmark (nmmmm.py), carried over as a
# starting point — it replaces the old 0.55 threshold, which was calibrated
# specifically for nomic-embed-text's score distribution and does not transfer to
# SapBERT's. Re-calibrate against real match scores on this KG before relying on it.
...
# Generates L2-normalized [CLS]-token SapBERT embeddings for a list of strings,
# batched for memory safety. Because output vectors are already L2-normalized,
# a plain dot product between two of them IS their cosine similarity — this is
# what the matching loop below relies on (torch.matmul instead of a separate
# cosine_similarity_manual step). Ported from the MindMap fuzzy-alignment
# benchmark (nmmmm.py).
def get_sapbert_embeddings(texts, model, tokenizer, batch_size=32):
    all_embeddings = []
    for i in range(0, len(texts), batch_size):
        ...
        with torch.no_grad():
            ...
            cls_rep = F.normalize(cls_rep, p=2, dim=1)
            all_embeddings.append(cls_rep.cpu())
...
if __name__ == "__main__":
    ...
    # Load the local SapBERT model once for the whole run — used to embed both
    # KG entity names (below) and each question's extracted keywords (in the
    # matching loop further down).
    print(f"Loading entity-matching embedding model '{EMBEDDING_MODEL_NAME}' onto {str(DEVICE).upper()}...", flush=True)
    sapbert_tokenizer = AutoTokenizer.from_pretrained(EMBEDDING_MODEL_NAME)
    sapbert_model = AutoModel.from_pretrained(EMBEDDING_MODEL_NAME, low_cpu_mem_usage=True).to(DEVICE)
    sapbert_model.eval()

    # KG-entity NAME list (see generate_entity_embeddings.py) — we only reuse the
    # entity names from this file now. The actual embedding vectors are recomputed
    # live with SapBERT (instead of loading precomputed nomic-embed-text vectors),
    # so they live in the same embedding space as the per-question keyword
    # embeddings computed below.
    ...
    print(f"Vectorizing {len(entity_names)} KG entity names with SapBERT...", flush=True)
    entity_embeddings_matrix = get_sapbert_embeddings(entity_names, sapbert_model, sapbert_tokenizer)
    ...
    for input_text_0 in TEST_QUESTIONS:
        ...
        # Entity matching via local SapBERT embeddings — MindMap's fuzzy
        # cosine-similarity matching, with the entity-name side
        # embedded live at runtime instead of a precomputed closed-vocabulary
        # lookup. All keywords for this question are embedded together in one
        # batch, then matched against the KG entity matrix via a single matrix
        # multiply (vectors are already L2-normalized, so the dot product IS
        # cosine similarity). A keyword is skipped entirely (not force-matched)
        # if even its best candidate falls below MIN_MATCH_SIMILARITY — see that
        # constant's comment for why.
        query_matrix = get_sapbert_embeddings(question_kg, sapbert_model, sapbert_tokenizer)
        similarity_matrix = torch.matmul(query_matrix, entity_embeddings_matrix.T)

        match_kg = []
        for i, kg_entity in enumerate(question_kg):
            cos_similarities = similarity_matrix[i].clone()

            max_index = torch.argmax(cos_similarities).item()
            match_kg_i = entity_names[max_index]

            while match_kg_i.replace(" ", "_") in match_kg:
                cos_similarities[max_index] = -1.0
                max_index = torch.argmax(cos_similarities).item()
                match_kg_i = entity_names[max_index]

            best_score = cos_similarities[max_index].item()
            if best_score < MIN_MATCH_SIMILARITY:
                print(f"  Note: '{kg_entity}' best candidate was '{match_kg_i}' at "
                      f"similarity {best_score:.3f}, below "
